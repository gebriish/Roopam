...
void _window_pos_callback(GLFWwindow* window, int xPos, int yPos) {
	Event move_event;
	move_event.type = EVENT_WINDOW_MOVE;
	move_event.window_move_data = { xPos, yPos };
	g_EventQueue.push(move_event);
}
...
Window* windowInitialize(int width, int height, const char* title, uint32_t flags) {
	...
	glfwSetWindowPosCallback(window->handle, _window_pos_callback);
...
void windowSetMode(const Window* window, WindowFlags flags) 
{
	...
	if(flags & WINDOWFLAG_VSYNC) 
		glfwSwapInterval((flags & WINDOWFLAG_VSYNC) ? 1 : 0);

	if (flags & WINDOWFLAG_MAXIMIZED)
